...
use crate::token::{create_empty_token_file, update_token_in_file};
                ...
                Ok(resp) => {
                    if resp.status().is_success() {
                        match resp.text().await {
                            Ok(login_response) => {
                                result.set(Some(Ok(login_response.clone()))); // Corrected line
                                let _ = create_empty_token_file();
                                let _ = update_token_in_file(&login_response);
                            }
                            Err(e) => {
